...
export const messages: Record<Locale, MessageDictionary> = {
  'zh-CN': {
    ...
    emptyState: {
      ...
      activeDescription: '当前会话已启用本地双向消息流，后续步骤再接入真实 AI。',
      messageAreaLabel: '消息区域',
      messageAreaTitle: '开始发送你的第一条消息',
      inputDescription: '发送后会自动追加一条本地假 AI 回复。',
  ...
  ja: {
    ...
    emptyState: {
      ...
      activeDescription: '現在の会話ではローカルの双方向メッセージフローが使え、次の段階で実際の AI に置き換えます。',
      messageAreaLabel: 'メッセージ領域',
      messageAreaTitle: '最初のメッセージを送信してください',
      inputDescription: '送信後にローカルの仮 AI 返信を自動で追加します。',
